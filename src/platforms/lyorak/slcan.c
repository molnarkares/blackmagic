...
void slcan_usb_out_cb(usbd_device *dev, uint8_t ep)
{
	...
	//usbd_ep_write_packet(usbdev, ep, buf, CDCACM_PACKET_SIZE);
	for(int i = 0; i < len; i++) {
...
void slcan_usb_in_cb(usbd_device *dev, uint8_t ep)
{
	(void) dev;
	(void) ep;
}
